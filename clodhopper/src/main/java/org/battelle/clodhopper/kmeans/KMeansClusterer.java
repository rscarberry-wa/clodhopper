...
public class KMeansClusterer extends AbstractClusterer {
	...
	public List<Cluster> doTask() throws Exception {
		...
		try {
			...
			int progressSteps = 2*maxIterations;
						
			ProgressHandler ph = new ProgressHandler(this, progressSteps);
			...
			// The trivial case.  No work to do.
			if (actualClusterCount == 1) {
				...
				int workerCount = params.getWorkerThreadCount();
				if (workerCount <= 0) {
            ...
            if (o != null && o.getClass() == this.getClass()) {
                ...
                if (other.sizes.length == this.sizes.length && other.members.length == this.members.length) {
                    ...
                    for (int i=0; i<this.sizes.length; i++) {
                        if (other.sizes[i] != this.sizes[i]) return false;
                        int lim = n + sizes[i];
                        for (int j=n; j<lim; j++) {
                            if (other.members[j] != this.members[j]) return false;
                        }
                        n += sizes[i];
                    }
                    return true;

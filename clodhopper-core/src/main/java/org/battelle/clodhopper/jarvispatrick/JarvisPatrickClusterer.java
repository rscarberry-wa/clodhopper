...
public class JarvisPatrickClusterer extends AbstractClusterer {
  ...
  protected List<Cluster> doTask() throws Exception {
    ...
    // tupleCount*(tupleCount - 1)/2 for the nested i-j for-loops for assignment.
    ...
    if (nearestNeighborsToExamine > tupleCount - 1) {
      ...
      ph.postMessage(String.format("reducing nearest neighbors to examine to %d, the number of possible neighbors", tupleCount - 1));
    ...
    if (nearestNeighborOverlap > nearestNeighborsToExamine) {
      ph.postMessage(String.format("reducing nearest neighbor overlap to %d, the number of nearest neighbors to examine",
    ...
    for (int i=0; i<tupleCount; i++) {
      ...
      for (int j=i+1; j<tupleCount; j++) {
        ...
        // If they're already both in the same cluster, there's nothing to do.
        //
        if (icluster < 0 || jcluster < 0 || icluster != jcluster) {
          ...
          if (overlapAtLeast(inearestNeighbors, jnearestNeighbors, nearestNeighborOverlap)) {            
            // If they're already assigned to different clusters, merge them.
            if (icluster >= 0 && jcluster >= 0) {
              icluster = mergeClusters(icluster, jcluster, clusterMemberships, clusterAssignments);
              ...
              icluster = jcluster;
              // Assign them both to a new cluster, since neither are in a cluster already.
            } else {
    ...
    // There still may be unassigned tuples, because they didn't have a large enough nn overlap
  ...
  private void assignToCluster(int index, int cluster, List<TIntArrayList> clusterMemberships) {
    clusterMemberships.get(cluster).add(index);
  }
  
  private int mergeClusters(int index1, int index2, List<TIntArrayList> clusterMemberships, int[] clusterAssignments) {
  ...
  private int[][] computeNearestNeighbors(ProgressHandler ph, int nearestNeighborsToExamine) {
    
    final int tupleCount = tuples.getTupleCount();
    int[][] nearestNeighbors = new int[tupleCount][];
    
    // A KD-Tree provides an efficient way of quickly looking up nearest neighbors.
